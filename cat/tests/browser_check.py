...
def main():
    ...
    with sync_playwright() as p:
        ...
        def pose(t):
            page.evaluate('(t) => catDemo.seek(t)', t)
        ...
        assert zero == pose(1.2), 'Loop seam differs'
        checks.extend(['native walk pose changes', '1.2-second loop closes'])
        ...
        frames = []
        for i in range(36):
            frames.append(Image.open(BytesIO(pose(i/30))).convert('RGB'))
